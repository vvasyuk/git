...
object Anagrams {
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = (dictionary map (word => (wordOccurrences(word), word))) groupBy (n => n._1) mapValues (x => x map (y => y._2)) withDefaultValue List()
