...
def get_node(df):
    root = TreeNodeOld("root")
    all_items = {}

    def _get_node(id):
        if id not in all_items:
            all_items[id] = TreeNodeOld(id=id)
        return all_items[id]
    ...
    def _generate_parent(node):
        if not node.has_parent and str(node.id).__contains__('.'):
            ...
            if new_parent_id_str in all_items:
                ...
                parent_obj = TreeNodeOld(id=new_parent_id_str, wap_id=node.wap_id, name=node.id, currency='',
    ...
    for index, row in df.iterrows():
        ...
        if id in all_items:
            ...
            node_obj.currency = ''
        if id not in all_items:
            all_items[id] = TreeNodeOld(id=id, wap_id=wap_id, name=name, currency='', shortName='', sector=sector, account=account, cust_name=cust_name, has_parent=True)
        if parent not in all_items:
            if parent is None and sector is not None and account is not None and cust_name is not None:
                ...
                all_items[parent] = TreeNodeOld(id=parent, wap_id=wap_id, name=name, currency='', shortName='', sector=sector, account=account, cust_name=cust_name, has_parent=False)
