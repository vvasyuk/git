...
object arrayBufferTest {
  def main(args: Array[String]): Unit = {
    ...
    // modify
    val ar1 = ArrayBuffer(1, 2);
    ar1(1) = 22;
    assert(ar1(1) == 22)
    val ar2 = ArrayBuffer(1, 2);
    ar2.update(1, 22);
    assert(ar2(1) == 22)
    ar2 += 3;
    assert(ar2 == ArrayBuffer(1, 22, 3)) // append (append)
    // +=: - updates in place
    // +:  - returns new collections with update
    0 +=: ar2;
    assert(ar2 == ArrayBuffer(0, 1, 22, 3)) // prepend (prepend)
    ar2 ++= Seq(4, 5);
    assert(ar2 == ArrayBuffer(0, 1, 22, 3, 4, 5)) // appendAll
    ar2 ++=: ar1;
    assert(ar1 == ArrayBuffer(0, 1, 22, 3, 4, 5, 1, 22)) // prependAll
    ar2.insert(1, 11);
    assert(ar2 == ArrayBuffer(0, 11, 1, 22, 3, 4, 5)) // insert
    ar2 -= 11;
    assert(ar2 == ArrayBuffer(0, 1, 22, 3, 4, 5)) // insert
